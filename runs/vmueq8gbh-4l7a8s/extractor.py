...
class CacheDocExtractor:
    ...
    def parse_source_code(self, source_code: str):
        ...
        # Aborda a interpolação dinâmica normalizando para placeholders legíveis
        key_matches = re.findall(r'(?:cache|redis)[._\w]*\b.*?[\'"]([^\'"]*[:*][^\'"]*)[\'"]', source_code, re.IGNORECASE)
        for k in key_matches:
            self.cache_patterns.add(k)

        # Captura genérica de strings parecidas com chaves Redis em atribuições
        ...
        # Captura de chamadas directas ao comando EXPIRE via cliente redis-py
    ...
    def parse_config(self, config_content: str):
        # Extração de diretivas do redis.conf (ex: maxmemory-policy, save, etc.)
        policy_match = re.search(r'^\s*maxmemory-policy\s+([a-zA-Z0-9_-]+)', config_content, re.MULTILINE)
        if policy_match:
            self.invalidation_rules.append({"command": "CONFIG_POLICY", "target": policy_match.group(1)})

    def generate_markdown(self) -> str:
        # Equívoco evitado: Não gera dump cru de JSON, mas sim tabela legível para humanos.
        md = "# Relatório Automatizado de Camada de Cache e Invalidação\n\n"
        
        md +="## 1. Padrões de Chaves Monitoradas (Namespaces)\n"
        if self.cache_patterns:
            md += "| Padrão / Namespace |\n|--------------------|\n"
            for p in sorted(self.cache_patterns):
                md += f"| `{p}` |\n"
        else:
            md += "_Nenhum padrão estático de chave detectado._\n"
        
        md += "\n## 2. Configurações de TTL\n"
        if self.ttls:
            md += "| Chave / Escopo | TTL (Segundos) |\n|----------------|----------------|\n"
            for k, v in self.ttls.items():
                md += f"| `{k}` | {v} |\n"
        else:
            md += "_Nenhum TTL explícito extraído._\n"

        md += "\n## 3. Estratégias e Gatilhos de Invalidação\n"
        if self.invalidation_rules:
            md += "| Comando / Política | Alvo |\n|--------------------|------|\n"
            for rule in self.invalidation_rules:
                md += f"| `{rule['command']}` | `{rule['target']}` |\n"
        else:
            md += "_Nenhuma regra de invalidação encontrada._\n"
