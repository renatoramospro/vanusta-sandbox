...
def test_concurrent_execution_overlap():
    ...
    Identificação de tarefas corrigida por meio de closure explícita para evitar colisão.
    """
    execution_timestamps = {}

    def make_slow_task(name, sleep_time):
        def task_func(parent_results=None):
            start = time.time()
    ...
    dag = WorkflowDAG()
    dag.add_task(Task("T1", make_slow_task("T1", 0.15)))
    dag.add_task(Task("T2", make_slow_task("T2", 0.15)))
