...
def execute_with_retry(service_name: str, config: RetryConfig, metrics: ExternalCallMetrics, func, *args, **kwargs):
    """
    Executa uma chamada externa aplicando Retry Exponencial com Full Jitter.
    """
    attempt = 0
    while True:
        attempt += 1
        ...
        try:
            ...
            # sleep = uniform(0, min(max_delay, base_delay * 2^(attempt-1)))
            exponential_backoff = min(config.max_delay, config.base_delay * (2 ** (attempt - 1)))
            sleep_time = random.uniform(0, exponential_backoff)
            
            logger.warning(f"[{service_name}] Erro transitório: {e}. Aguardando {sleep_time:.3f}s antes do retry...")
            time.sleep(sleep_time)
        except PermanentError as e:
            metrics.failures += 1
            logger.error(f"[{service_name}] Erro permanente detectado. Abortando retries. Erro: {e}")
            raise e
...
if __name__ == "__main__":
    ...
    # Utilizando uma lista mutável para contornar o escopo global sem usar 'nonlocal' incorretamente
    state = {"call_attempts": 0}
    def unstable_service():
        state["call_attempts"] += 1
        if state["call_attempts"] < 3:
    ...
    assert metrics.attempts == 3 + 1, f"Número total de tentativas incorreto (esperado 4, obtido {metrics.attempts})."
