...
# 1. Simulação de 30 Projetos Piloto avaliados em 4 dimensões (escala Likert de 1 a 5)
# Dimensões: D1 (Monitoramento Proativo), D2 (Flexibilidade Estrutural), 
#           D3 (Agilidade Orçamentária), D4 (Competência Regulatória)
n_projetos = 30
n_itens = 8

# Gerando dados sintéticos realistas para as respostas dos itens dos questionários (30 projetos x 8 itens)
...
def calcular_alfa_cronbach(df):
    ...
    n = df.shape[1]
    alfa = (n / (n - 1)) * (1 - (item_vars.sum() / total_var))
    return alfa

alfa_cronbach = calcular_alfa_cronbach(df_itens)
print(f"[Métrica] Alfa de Cronbach calculado: {alfa_cronbach:.3f} (Meta: >= 0.80)")

# 3. Contraemplo do Equívoco Comum: Métrica baseada puramente em volume documental vs Adaptabilidade Real
# Equívoco: Achar que quanto mais relatórios de compliance gerados, maior a adaptabilidade.
volume_documental = np.random.randint(50, 200, size=n_projetos) # quantidade de páginas de relatórios
sucesso_real_projeto = 0.4 * df_itens.mean(axis=1) + np.random.normal(0, 0.1, n_projetos) # sucesso baseado na adaptabilidade real
...
# Correlação do framework proposto com o sucesso de entrega sob mudança regulatória
...
# Simulando respostas de 30 gestores (True = considerou útil, False = não considerou)
