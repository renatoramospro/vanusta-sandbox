...
# Gerando dados sintéticos determinísticos com base em random
dados_itens = [[random.randint(2, 5) for _ in range(n_itens)] for _ in range(n_projetos)]
...
# Sucesso real inversamente ou fracamente correlacionado com volume documental excessivo
sucesso_real_projeto = [max(0.2, min(1.0, 0.8 - (v / 400) + random.uniform(-0.1, 0.1))) for v in volume_documental]
...
utilidade_gestores = [random.random() < 0.78 for _ in range(n_projetos)]
